			...
			<div className='px-3 py-4 flex flex-col gap-2'>
				...
				<div className=''>
					<div className='text-[#132339] leading-[22px] dark:text-white'>
						{i18n.language === 'uz' ? (
							<>{data?.adults_count + data?.children_count} ta sayyoh</>
						) : (
							<>
								За {data?.adults_count + data?.children_count}х {t('home.adult2')}
							</>
						)}
					</div>
				...
				<div className='text-[#235DFF] leading-[22px] font-medium'>
					от {data?.price_discounted?.toLocaleString('en-US').replace(/,/g, ' ')} {t('home.uzs')}
				</div>
			</div>
		</div>
	);
