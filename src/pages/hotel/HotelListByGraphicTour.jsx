...
const HotelListByGraphicTour = ({ darkmode }) => {
	...
	const fetchData = searchParams => {
		Axios()
			.get(`api/v1/somo-travel/tour-prices-content2${searchParams === undefined ? window.location.search : '?' + searchParams}&page=${page}&per_page=3`)
	...
	const handleFind = () => {
		...
		if (t) {
			...
			navigate(`/hotels/result-graphic-tour/?${searchParamss}`);
		} else {
			setObjError(error);
			window.scrollTo({
				top: 0,
...
export default HotelListByGraphicTour;
